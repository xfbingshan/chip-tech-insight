...
from src.reporting.template_config import load_template
...
class PPTGenerator:
    ...
    def generate_flash_brief(
        self, content: Dict[str, Any], docs: List[Dict[str, Any]]
    ) -> str:
        """生成技术快讯（单页）"""
        self.prs = Presentation()
        self.prs.slide_width = Inches(self.template.slide_width)
        ...
        title_box = slide.shapes.add_shape(
            ...
            Inches(0),
            Inches(0),
            Inches(self.template.slide_width),
            Inches(1.2),
        )
        ...
        # 日期标签
        date_label = slide.shapes.add_textbox(
            Inches(10.5), Inches(0.35), Inches(2.5), Inches(0.5)
        )
        ...
        for item in innovations:
            self._add_bullet_text(
                slide, f"• {item}", Inches(0.7), Inches(y_pos), width=Inches(12)
            )
            y_pos += 0.5
        ...
        self._add_bullet_text(
            slide, trl_text, Inches(0.7), Inches(y_pos), width=Inches(12)
        )
        ...
        self._add_bullet_text(
            slide, ", ".join(players), Inches(0.7), Inches(y_pos), width=Inches(12)
        )
        ...
        advice_tf.text = (
            f"💡 建议：{content.get('一句话建议', '建议持续关注该技术方向')}"
        )
        ...
        y_pos += 1.3
        sources = [
            f"{d.get('source', '')}: {d.get('title', '')[:40]}..." for d in docs[:3]
        ]
        self._add_bullet_text(
            slide,
            "来源：" + "; ".join(sources),
    ...
    def _create_cover(self, content: Dict):
        ...
        bg = slide.shapes.add_shape(
            ...
            0,
            Inches(self.template.slide_width),
            Inches(self.template.slide_height),
        )
        bg.fill.solid()
        ...
        conclusion = slide.shapes.add_textbox(
            Inches(1), Inches(4.2), Inches(11.333), Inches(1.0)
        )
    ...
    def _create_content_slide(self, title: str, text: str):
        ...
        content = slide.shapes.add_textbox(
            Inches(0.7), Inches(1.2), Inches(12), Inches(5.8)
        )
    ...
    def _create_list_slide(self, title: str, items: List[str]):
        ...
        for item in items:
            self._add_bullet_text(
                slide, f"• {item}", Inches(0.7), Inches(y), width=Inches(12)
            )
            y += 0.6
    ...
    def _add_bullet_text(
        self, slide, text: str, left, top, width=Inches(12), font_size=14, color=None
    ):
        if color is None:
