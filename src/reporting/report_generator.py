...
class ReportGenerator:
    ...
    def generate_flash_brief(
        ...
        lines.append(
            f"**情报来源**: {', '.join(set(d.get('source', '') for d in docs))}\n"
        )
        lines.append("---\n")
        ...
        lines.append("## 行动建议\n")
        lines.append(
            f"> 💡 **{content.get('一句话建议', '建议持续关注该技术方向')}**\n"
        )
        ...
        for d in docs[:5]:
            lines.append(
                f"- [{d.get('title', '')}]({d.get('pdf_url', '')}) ({d.get('source', '')})"
            )
    ...
    def generate_org_brief(self, org_name: str, docs: List[Dict[str, Any]]) -> str:
        ...
        lines.append(
            f"**涉及方向**: {', '.join(sorted(set(d.get('assessment', {}).get('category', 'Other') for d in docs)))}\n"
        )
        lines.append("---\n")
        ...
        for i, doc in enumerate(docs, 1):
            ...
            lines.append(
                f"- **来源**: [{doc.get('source', '')}]({doc.get('pdf_url', '')})"
            )
            lines.append("")
        ...
        lines.append(
            f"{org_name} 近期在 **{top_cat}** 方向布局最为密集，建议重点关注其技术路线与产品化节奏。\n"
        )
