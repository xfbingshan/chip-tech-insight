        ...
        for source in self.sources:
            try:
                ...
                if channel is not None:
                    ...
                    for item in items:
                        published = self._parse_rss_date(self._get_text(item, "pubDate"))
                        if published:
                            if published.tzinfo is None:
                                published = published.replace(tzinfo=__import__('datetime').timezone.utc)
                            if published < cutoff:
                                continue
                        ...
                        results.append({
                            ...
                            "abstract": self._clean_html(self._get_text(item, "description")),
                            "authors": [],
                    ...
                    for entry in root.findall("atom:entry", ns)[:self.max_entries]:
                        published_str = self._get_atom_text(entry, "atom:published", ns) or self._get_atom_text(entry, "atom:updated", ns)
                        try:
                            published = datetime.fromisoformat(published_str.replace("Z", "+00:00")) if published_str else datetime.now(__import__('datetime').timezone.utc)
                        except:
                            published = datetime.now(__import__('datetime').timezone.utc)
                        if published < cutoff:
                            continue
                        
                        results.append({
                            "id": self._get_atom_text(entry, "atom:id", ns),
                            "title": self._clean_html(self._get_atom_text(entry, "atom:title", ns)),
                            "abstract": self._clean_html(self._get_atom_text(entry, "atom:summary", ns)),
                            "authors": [],
